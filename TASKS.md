...
**Status:** All Priority 1–3 tasks implemented, tested (92 tests, 82% coverage), and shipped to GitHub.
...
- [x] **`pipeline/generator.py`** — Add `get_langfuse_handler()` (or import from `core/observability.py`); inject `callbacks=[langfuse_handler]` into every `ChatOpenAI` construction.
  - Acceptance: `grep -n "ChatOpenAI(" pipeline/generator.py` shows `callbacks=` on every match.
- [x] **`pipeline/generator.py`** — All `chain.ainvoke()` or `llm.ainvoke()` calls include `config={"callbacks": get_callbacks()}`.
  - Acceptance: No LLM invocation in `pipeline/generator.py` lacks a callback config.
- [x] **`orchestrator/`** (all node files) — All LLM calls in CRAG nodes (grade, generate, rewrite) include Langfuse handler.
  - Acceptance: Each orchestrator node that calls an LLM passes the handler.
- [x] **`core/observability.py`** (create) — Centralize `get_langfuse_handler()` and `get_callbacks()` here so no duplication across files.
  - Acceptance: File exists; `pipeline/generator.py` and all orchestrator files import from it.
- [x] **`.env.template`** — Add `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_HOST`.
  - Acceptance: All three keys present in `.env.template` with comments.
- [x] **`requirements.txt`** — Add `langfuse>=2.0.0`.
  - Acceptance: `python -c "import langfuse"` exits 0 after install.
- [x] **`tests/conftest.py`** — Add `mock_langfuse` and `mock_observability` fixtures.
...
- [x] **`docker-compose.yml`** (create at repo root) — Developer convenience compose with `env_file: .env`, build context `.`, `deploy/Dockerfile`. Services: `api`, `vectorstore` (qdrant/pgvector), `langfuse`, `langfuse_db`.
  - Acceptance: `docker compose config` exits 0; `docker compose up` starts API on port 8000.
- [x] **`README.md`** — Update "Getting Started" to use `docker compose up` from root.
  - Acceptance: No instruction to `cd deploy/` in the README quick start section.
- [x] **`.env.template`** — Add `LANGFUSE_NEXTAUTH_SECRET`, `LANGFUSE_SALT`.
...
- [x] **`docs/architecture.md`** — Add documentation for `orchestrator/` (CRAG pattern, graph flow, node names).
  - Acceptance: Section "orchestrator/" is present with CRAG flow diagram and state TypedDict description.
- [x] **`docs/architecture.md`** — Add documentation for `embeddings/` (nim_embedder.py, chunker.py).
  - Acceptance: Section "embeddings/" is present with env var configuration documented.
- [x] **`docs/architecture.md`** — Add documentation for `vectorstore/` (factory, faiss_store, pgvector_store).
  - Acceptance: Section "vectorstore/" is present with backend selection via `VECTORSTORE_BACKEND` documented.
- [x] **`docs/architecture.md`** — Add documentation for `retriever/` (hybrid_retriever.py, fusion method).
  - Acceptance: Section "retriever/" is present with description of dense+sparse fusion.
- [x] **`docs/architecture.md`** — Add complete Environment Variables table.
  - Acceptance: All env vars in `.env.template` appear in the architecture doc table.
- [x] **`docs/architecture.md`** — Add High-Level Data Flow diagram (ASCII or mermaid).
...
- [x] **`backends/`** or **`vectorstore/`** — Define `VectorStoreBase` ABC with abstract methods: `add_documents`, `similarity_search`, `delete`. Have `pgvector.py` and `milvus.py` implement it.
...
- [x] **`core/exceptions.py`** (create) — Define `IngestionError`, `EmbeddingError`, `RetrievalError`, `GenerationError` as subclasses of `Exception`.
...
- [x] **`.github/workflows/ci.yml`** — Add `--cov-fail-under=80` to pytest command.
  - Acceptance: CI fails if coverage for `pipeline/`, `embeddings/`, `retriever/`, `orchestrator/` drops below 80%.

### 2.4 — RAGAS Baseline Results

- [x] **`evals/results/`** (create directory) — Run a baseline RAGAS eval against the sample dataset and commit the results JSON as the baseline.
...
- [x] **`embeddings/chunker.py`** — Verify `CHUNK_SIZE` and `CHUNK_OVERLAP` are read from environment (not hardcoded). Add fallback defaults with logging.
...
- [x] **`vectorstore/faiss_store.py`** — Add `logger.warning("Using FAISS in-memory store — data will not persist across restarts. Not suitable for production.")` in the `__init__` method.
  - Acceptance: Starting the app with `VECTORSTORE_BACKEND=faiss` logs this warning.

### 2.7 — Health Check Enhancement

- [x] **`api/`** — Ensure `GET /health` returns `{"status": "ok", "nim_reachable": bool, "vectorstore_backend": str}`.
...
- [x] **`ingestion/`** — Wrap all synchronous loaders with `asyncio.to_thread` at the `pipeline/indexer.py` layer for non-blocking ingestion.
...
- [x] **`evals/ragas_runner.py`** — Support both `.json` (list of dicts) and `.jsonl` (newline-delimited) dataset formats.
  - Acceptance: `--dataset` flag accepts both formats without additional flags.

### 3.3 — k8s Helm Chart

- [x] **`deploy/helm/`** (create) — Basic Helm chart wrapping the k8s manifests in `deploy/k8s/`.
  - Acceptance: `helm lint deploy/helm/` passes. Values file documents all configurable parameters.

### 3.4 — ISV Customization Guide

- [x] **`docs/isv-customization.md`** (create) — Step-by-step guide for ISV partners: how to swap the vector backend, add a new ingestion loader, change the embedding model, and modify the CRAG graph.
  - Acceptance: Document covers all four customization scenarios with code examples.

### 3.5 — Add Reranker Support

- [x] **`retriever/hybrid_retriever.py`** — Add optional reranking step after hybrid retrieval using a NIM reranker model (controlled by `USE_RERANKER=true` env var).
...
- [x] **All repos** — Confirm `langfuse>=2.0.0` is in each repo's `requirements.txt`.
- [x] **All repos** — Confirm `docker-compose.yml` exists at each repo root.
- [x] **All repos** — Confirm `.env.template` documents `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_HOST`.
- [x] **All repos** — Confirm `ruff check . && mypy .` passes clean in each repo independently.
- [x] **All repos** — Confirm LangGraph state is `TypedDict` (not `dict`) in every graph file.
- [x] **All repos** — Confirm no `from langchain.` imports (must be `langchain_core`, `langchain_openai`, or `langchain_community`).
